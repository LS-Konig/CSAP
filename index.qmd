...
All models are intended to be estimated using Bayesian inference via Hamiltonian Monte Carlo implemented in Stan through the `brms` package in R [@burkner2017brms]. We plan to use weakly informative priors: $\mathcal{N}(0,10)$ for fixed effects, $\text{Exponential}(1)$ for random-effect standard deviations, and $\text{Half-}\mathcal{N}(0,5)$ for residual variance. Convergence will be evaluated using $\hat{R}$ and effective sample sizes. Results are summarized using posterior medians and 95% credible intervals, and hypotheses are evaluated through posterior probabilities.
