...
For explicit and implicit partisans alike, the identified party—whether derived from attachment or electoral preference—serves as the partisan anchor defining copartisan versus outpartisan distinctions in subsequent evaluations. This procedure distinguishes subjective identification from behavioral anchoring while maintaining a consistent reference point for measuring affective polarization across respondents.

Importantly, the distribution of partisan types varies substantially across countries. @fig-impl-expl-distribution displays this cross-national variation, with explicit partisans comprising roughly 40% of respondents in some countries and more than 80% in others. Consequently, identical sampling decisions -- such as restricting analyses to self-reported identifiers -- produce different populations of inference across national contexts, motivating our comparison of affective polarization across partisan types.

{{< embed code/03_explanal/3.1_descr_anal.qmd#fig-impl-expl-distribution >}}

### Analytical Strategy {#sec-methods}

We (plan to) employ Bayesian hierarchical models to estimate affective polarization (AP) as a function of partisan type, accounting for the nested data structure and enabling principled cross-national inference. The empirical strategy is designed in three stages corresponding to the main analyses and planned extensions.

#### Stage 1: Core Model — Partisan Type and Affective Polarization {#sec-core-model}

The primary analysis evaluates whether explicit and implicit partisans differ in vertical versus horizontal AP. The core model specification is
...
Here, $\text{Outcome}_{ric}$ denotes either the thermometer rating (vertical AP) or token allocation (horizontal AP) for observation $r$ by respondent $i$ in country $c$. $\text{CoPartisan}_{ric}$ indicates whether the evaluated party or profile matches respondent $i$’s partisan anchor, and $\text{Type}_i$ distinguishes explicit ($=1$) from implicit ($=0$) partisans. The coefficient $\beta_1$ captures baseline ingroup favoritism among implicit partisans, $\beta_2$ represents baseline affective differences between partisan types, and $\beta_3$ is the key parameter capturing whether explicit partisans exhibit stronger AP than implicit partisans.

This specification treats AP as a derived quantity of interest defined as the expected difference between evaluations of copartisans and outpartisans. For respondent $i$ of partisan type $t$,

$$
\text{AP}_t = \mathbb{E}[\text{Outcome} \mid \text{CoPartisan}=1, \text{Type}=t] - \mathbb{E}[\text{Outcome} \mid \text{CoPartisan}=0, \text{Type}=t].
$$

For implicit partisans $(t=0)$, $\text{AP}_{\text{implicit}}=\beta_1$, whereas for explicit partisans $(t=1)$, $\text{AP}_{\text{explicit}}=\beta_1+\beta_3$. The hypotheses therefore translate into predictions about $\beta_3$: identity-based mechanisms imply $\beta_3>0$ for both vertical and horizontal AP, whereas categorization-based mechanisms imply $\beta_3\approx0$ in the respective dimension. This framework remains consistent with the logic of the Affective Polarization Index while extending it to behavioral outcomes and evaluating whether subjective attachment amplifies polarization beyond partisan anchoring.

#### Stage 2: Multilevel Extension for Cross-National Comparison {#sec-multilevel}

The pooled specification provides population-average effects but may conceal cross-national heterogeneity. To examine variation across European democracies, the model is extended to a three-level hierarchical structure:
...
&(\beta_3 + \beta_{3c}) \cdot (\text{CoPartisan}_{ric} \times \text{Type}_i) + \varepsilon_{ric}.
\end{aligned}
$$

Within-respondent variation across evaluations is captured by $\varepsilon_{ric} \sim \mathcal{N}(0,\sigma^2)$. Respondent-level heterogeneity is modeled through $\alpha_i \sim \mathcal{N}(0,\sigma_{\text{respondent}}^2)$, while country-level clustering is captured by $\alpha_c \sim \mathcal{N}(0,\sigma_{\text{country}}^2)$. Random slopes are specified at the country level: $\beta_{1c}$ captures cross-national variation in AP strength, $\beta_{2c}$ variation in baseline differences between partisan types, and $\beta_{3c}$ variation in the partisan-type gap in AP.

This hierarchical specification enables two complementary inferential perspectives. Marginalizing over random effects yields pooled estimates representing average patterns across European democracies. Conditioning on country-level effects produces country-specific estimates,

$$
\text{AP}_{c,t} = (\beta_1 + \beta_{1c}) + t \cdot (\beta_3 + \beta_{3c}),
$$

allowing evaluation of whether the vertical–horizontal asymmetry in partisan type effects is consistent across party systems or varies systematically across contexts.

#### Stage 3: Individual-Level Moderators {#sec-moderators}

A planned extension examines whether the explicit–implicit gap in AP varies with respondent characteristics, corresponding to hypotheses about individual-level moderators. The model incorporates covariates and three-way interactions:
...
The coefficient $\beta_7$ evaluates whether the partisan-type gap in AP varies systematically with individual characteristics. Covariates will be selected based on established correlates of affective polarization, including ideological extremity, political interest, social identity strength, perceived threat, and democratic norms. Separate models will be estimated for vertical and horizontal outcomes to assess whether moderation emerges for attitudinal polarization but not behavioral discrimination. Substantive interpretation will rely on simulated conditional marginal effects across observed covariate ranges.

#### Estimation and Inference {#sec-estimation}

All models are estimated using Bayesian inference via Hamiltonian Monte Carlo implemented in Stan through the `brms` package in R. Weakly informative priors are specified: $\mathcal{N}(0,10)$ for fixed effects, $\text{Exponential}(1)$ for random-effect standard deviations, and $\text{Half-}\mathcal{N}(0,5)$ for residual variance. Posterior sampling uses four chains of 2,000 iterations with 1,000 warmup iterations, yielding 4,000 posterior draws. Convergence is evaluated using $\hat{R}<1.01$ and effective sample sizes exceeding 1,000. Results are summarized using posterior medians and 95% credible intervals, and hypotheses are evaluated through posterior probabilities.

Substantive quantities are derived through posterior simulation. For each draw, AP by partisan type is calculated as $\text{AP}_t=\beta_1+t\cdot\beta_3$, and country-specific estimates as $\text{AP}_{c,t}=(\beta_1+\beta_{1c})+t\cdot(\beta_3+\beta_{3c})$. For moderator models, the conditional partisan-type difference is $\Delta\text{AP}=\beta_4+\beta_7\cdot\text{Covariate}$. This simulation-based approach propagates parameter uncertainty through all derived quantities.

Full model specifications, prior sensitivity analyses, and diagnostics will be reported in the Online Appendix.
