...
abstract: |
  Affective polarization (AP) research faces a core measurement choice: should partisans be identified by self-reported attachment or by vote choice? The distinction is theoretical -- identity accounts imply attachment intensifies polarization, whereas categorization accounts imply vote anchors suffice -- and comparative, given wide cross-national variation in attachment rates across European democracies. We introduce partisan type (explicit partisans reporting attachment versus implicit partisans denying it yet revealing a vote anchor) and examine whether attachment differentially corresponds to vertical AP (party-directed attitudes) and horizontal AP (citizen-directed behavior). Analyzing thermometer ratings and conjoint trust-game data from 25 countries (N≈29,800), explicit partisans exhibit greater vertical AP than implicit partisans, while the groups display similar levels of horizontal behavioral discrimination. The pattern suggests that party-directed affect and citizen-directed behavior are not structured by the same individual-level disposition. For comparative research, combining vote-anchored and attachment-based samples appears appropriate for behavioral analyses, whereas restricting samples to explicit identifiers may overstate the behavioral implications of attitudinal polarization in multiparty systems.
execute:
  echo: false
  warning: false
  eval: true
