...
# THIS APPROACH NEED FIXING: NOBS IS REDUCED DUE TO COVARIATES IN MODELS

# adjust formula for glm
binomial_response_d <- df_modelvars |>
  filter(meta_game == "dict") |>
  select(cj_token) |>
  mutate(nontokens = 10 - cj_token)

binomial_response_t <- df_modelvars |>
  filter(meta_game == "trust") |>
  select(cj_token) |>
  mutate(nontokens = 10 - cj_token)
