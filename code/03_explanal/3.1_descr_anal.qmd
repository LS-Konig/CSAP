...
```{mermaid}
flowchart TD
    A("Respondents <br> N = 22,858")
        -->|40.9%| B("No Party Attachement <br> N = 9,349")
    A
        -->|59.1%| C("Party Attachement <br> (explicit) <br> N = 13,509")

    B -->|41.2%| D("No Vote reported <br> (non-partisan) <br> N = 3,854")
    B -->|58.8%| E("Vote reported <br> (implicit) <br> N = 5,495")

    C -->|8.0%| F("No Vote reported <br> N = 1,086")
    C -->|92.0%| G("Vote reported <br> N = 12,423")

    G -->|75.0%| H("Same as <br> Party Attachement <br> N = 9,314")
    G -->|25.0%| I("Diff from <br> Party Attachement <br> N = 3,109")
```

Distribution of respondents by reported party attachment and vote behavior. Percentages refer to the parent node.
:::
...
# column labels are taken from the data rather than hard-coded: the published
# `q_gender` is coded female / male / other, which is not the order the previous
# raw coding produced
gender_tab <- eu25games2019 |>
  distinct(meta_pid, .keep_all = T) |>
  tabyl(
    meta_country,
    q_gender
...
gender_tab |>
  knitr::kable(
    col.names = c("Country", str_to_title(names(gender_tab)[-1]))
  )
```
...
df_modelvars <- eu25games2019 |>
  filter(
    # co-national profiles only; the published coding calls this level
    # "own_country" (previously "co-national" in `der_conational`)
    cj_nationality_shown == "own_country",
  ...
  select(
    ...
    q_lr_self, # Left-Right ideological self-placement (Controls for core political position)
    q_euint_self, # EU integration position (Controls for major second dimension position)
    q_nativ_econ, # Attitudes towards economic nativism (Controls for key sociotropic threat perception)
    q_nativ_cult, # Attitudes towards cultural nativism (Controls for key sociotropic identity boundary)
    q_religion, # Respondent's religious affiliation (Controls for a major social cleavage/identity)
    ...
    q_satis_demo_nat, # Satisfaction with democracy in the country (System trust)
    q_nat_efficacy_understand, # Political efficacy (national level)
    q_nat_efficacy_care, # Political responsiveness (national level)
    q_nat_efficacy_nosay, # Political efficacy (national level)
    q_eu_efficacy_understand, # Political efficacy (EU level)
    q_eu_efficacy_care, # Political responsiveness (EU level)
    q_eu_efficacy_nosay, # Political efficacy (EU level)
    q_party_harm, # Anti-party sentiment ("Parties do more harm than good")
    q_pop_reps, # Anti-elitism ("Officials talk too much")
    q_pop_citizen, # People-centrism ("Prefer citizens to politicians")
    q_pop_people, # People-centrism ("People, not politicians, should decide")
    q_pop_will, # People-centrism ("Politicians must follow the people's will")
    q_pop_goodevil, # Manichaean worldview ("Politics is good vs. evil")
    q_people_incompetent, # System cynicism ("People are unaware")
    q_tech_leaders, # Anti-populism (Elitism)
    q_tech_experts, # Anti-populism (Expert governance)
    q_dem_listen, # Tolerance/Pluralism
    q_dem_compromise, # Tolerance/Pluralism

    # --- BLOCK 3: Political Engagement & General Identity (Predictors of T and Y) ---
    q_identity_country, # Emotional attachment to one's country
    q_identity_eu, # Emotional attachment to the EU
    q_identity_europe, # Emotional attachment to Europe
    q_nat_int, # Political engagement (national)
    q_eu_int, # Political engagement (EU)
    q_election_importance_self, # Political engagement (election importance)

    # --- BLOCK 4: Socioeconomic Status (SES) and Economic Context (Background Predictors) ---
    # NOTE: education enters as the age at which the respondent stopped
    # full-time education. The previous pipeline used an education-*level*
    # scale that Hahm et al. had constructed; the published data does not
    # carry it, and `q_edu_age_stop` is the closest available proxy. It is a
    # different construct, so read it as such rather than as a level measure.
    q_edu_age_stop, # Education (age left full-time education)
    q_class, # Subjective social class
    q_eval_fin_household, # Micro-level economic evaluation
    q_eval_job, # Job/Employment evaluation
    q_eval_econ_nat, # Macro-level economic evaluation (country)
...
# Recode don't-know answers and implausible values to NA.
#
# The published data codes the categorical background items as strings rather
# than as the numeric scales the previous pipeline received, and it has already
# stripped the numeric don't-know codes from the ordinal scales. What remains to
# clean is therefore the explicit `dont_know` / `other` string levels, plus the
# out-of-range values in the education item (age at which full-time education
# stopped runs from 0 to 120 in the raw data).
df_modelvars <- df_modelvars |>
  mutate(
    q_class = if_else(q_class %in% c("dont_know", "other"), NA, q_class),
    q_religion = if_else(q_religion == "dont_know", NA, q_religion),
    q_edu_age_stop = if_else(
      q_edu_age_stop >= 5 & q_edu_age_stop <= 70,
      q_edu_age_stop,
      NA
    )
  )

# Categorical to factors, ordered low -> high where the variable is ordinal
df_modelvars <- df_modelvars |>
  mutate(
    q_class = factor(
      q_class,
    ...
    q_religion = as_factor(q_religion)
  )

# The factor recodes above must not silently produce all-NA columns: that is what
# happens when the published coding changes and the `levels` are left stale.
stopifnot(
  all(vapply(
    df_modelvars[c("q_class", "q_rural_urban", "q_gender", "q_religion")],
    \(x) mean(is.na(x)) < 0.5,
    logical(1)
  ))
)

# Drop covariates that are too sparse to carry into a listwise-complete model
# frame. Determined from the data rather than hard-coded, so that a change in
# upstream item coverage is visible here instead of silently emptying the frame.
na_threshold <- 0.10

sparse_covars <- df_modelvars |>
...
sparse_covars |>
  ...
  knitr::kable(
    caption = paste0(
      "Covariates dropped for missingness above ",
      na_threshold * 100,
      "%."
    )
  )
...
df_modelvars <- df_modelvars |>
  select(-all_of(sparse_covars$Variable))

# standardize survey scales
df_modelvars <- df_modelvars |>
  mutate(
    across(
      # any_of(), because the sparsity filter above may already have removed
      # some of these
      .cols = any_of(c(
        "q_lr_self",
        "q_euint_self",
...
modelvars_labels <- c(
  ...
  "Game" = "meta_game_lab",
  "Round" = "meta_round",
  # survey wave, now on the published data's own scale: the games ran in
  # waves 2 and 3 (the previous pipeline relabelled these 1 and 2)
  "Survey wave" = "meta_wave",
  "LR Pos" = "q_lr_self_z",
  "EU Pos" = "q_euint_self_z",
  "Econ. Nativism" = "q_nativ_econ_z",
  "Cult. Nativism" = "q_nativ_cult_z",
  "Satis. Democ." = "q_satis_demo_nat_z",
  "Understand Nat. Pol." = "q_nat_efficacy_understand_z",
  "Understand EU Pol." = "q_eu_efficacy_understand_z",
  "Parties Harm" = "q_party_harm_z",
  "Officials Talk/Action" = "q_pop_reps_z",
  "Politics Good/Evil" = "q_pop_goodevil_z",
  "People Unaware" = "q_people_incompetent_z",
  "Leaders Educated" = "q_tech_leaders_z",
  "Expert Decisions" = "q_tech_experts_z",
  "Listen Other Groups" = "q_dem_listen_z",
  "Democ. Compromise" = "q_dem_compromise_z",
  "Interest Nat. Pol." = "q_nat_int_z",
  "Interest EU Pol." = "q_eu_int_z",
  "Eval. HH Finance" = "q_eval_fin_household_z",
  "Eval. Job" = "q_eval_job_z",
  "Eval. Nat. Econ." = "q_eval_econ_nat_z",
  "Eval. Eur. Econ." = "q_eval_econ_eur_z",
  "Risk Taking" = "q_risk_taking_z",
  "Future Discount" = "q_future_discount_z",
  # the published data carries age at which education stopped, not an
  # education-level scale -- see the note in the model-data chunk
  "Education, age left (Z)" = "q_edu_age_stop_z",
  "Age (Z)" = "q_age_z",
  "Religion" = "q_religion",
  "Perc. Class" = "q_class",
...
```{r}
#| label: fig-distr-token
#| fig-dpi: 500
#| fig-cap: "Distribution of token allocation by game. Dictator game: $Mean = 3.31$, $median = 4$, $SD = 2.32$. Trust game: $Mean = 3.37$, $median = 3$, $SD = 2.47$"
#| fig-pos: H

summary_stats <- df_modelvars |>
  group_by(meta_game_lab) |>
  summarise(
    mean_token = mean(cj_pl2, na.rm = TRUE),
    median_token = median(cj_pl2, na.rm = TRUE),
    sd_token = sd(cj_pl2, na.rm = TRUE)
  )


df_modelvars |>
  select(cj_pl2, meta_game_lab) |>
  ggplot(
    aes(x = cj_pl2, fill = meta_game_lab)
  ) +
  geom_histogram(
...
covariate_tab <- df_modelvars |>
  select(
    der_partisan_type,
    ends_with("_z"),
    any_of(c("q_religion", "q_class", "q_rural_urban", "q_gender"))
  )

# every standardized scale is continuous by construction; derive the list from
# the frame so that dropping a sparse covariate upstream cannot break this table
covariate_tab |>
  tbl_summary(
    by = der_partisan_type,
    missing = "no",
    type = as.list(setNames(
      rep("continuous", sum(str_ends(names(covariate_tab), "_z"))),
      str_subset(names(covariate_tab), "_z$")
    ))
  ) |>
