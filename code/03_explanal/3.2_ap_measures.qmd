...
#| fig-cap: "Distribution of thermometer ratings (rescaled from 0-100 to 0-10). $Mean = 3.82$, $median = 4$, $SD = 3.04$"
...
$$
\text{Spread}_i = \sqrt{ \frac{ \sum_{p=1}^{P} ( \text{like}_{ip} - \overline{\text{like}}_{i})^2 } {n_p - 1}}
$$

i.e. the ordinary (sample) standard deviation of respondent $i$'s party ratings, which
is what `sd()` computes in the chunk below. The denominator is $n_p - 1$, not $n_p$.
