...
**Affective Polarization Index** (Reiljan 2020):
...
```{r}
#| label: total-sample-ap-thermo

# Reiljan API unweighted

# thermo coparty indicator
eu25games2019 <- eu25games2019 |>
  mutate(
    thermo_co = case_when(
      ext_q_thermo_1_pf_name == der_partisan_anchor ~ 1,
      ext_q_thermo_2_pf_name == der_partisan_anchor ~ 2,
      ext_q_thermo_3_pf_name == der_partisan_anchor ~ 3,
      ext_q_thermo_4_pf_name == der_partisan_anchor ~ 4,
      ext_q_thermo_5_pf_name == der_partisan_anchor ~ 5,
      ext_q_thermo_6_pf_name == der_partisan_anchor ~ 6,
      .default = NA
    )
  )

eu25games2019 |>
  select(starts_with("q_thermo_")) |>
  select(-ends_with("party")) |>
  rowwise()
