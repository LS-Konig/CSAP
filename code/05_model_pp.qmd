...
#' @param re_formula_setting Passed to posterior_epred(); NULL = marginal REs,
#'        TRUE = include REs, NA = ignore all REs, or a formula.
#'
...
# 6. Function to plot cross-country CATE results
# ----------------------------------------------
...
plot_country_results <- function(country_qois_df) {
  cate_data <- country_qois_df |>
    ...
    mutate(
      Metric_Code = case_when(
        Quantity == "CATE_AP" ~ "Affective Polarization (CATE)",
        Quantity == "CATE_IF" ~ "Ingroup Favoritism (CATE)",
        Quantity == "CATE_OD" ~ "Outgroup Derogation (CATE)"
      ),
  ...
  #     linewidth = 0.8
  #   ) +
  #   facet_wrap(~Metric_Code, ncol = 3, scales = "free_y") +
  #   labs(
  #     x = "Conditional Average Treatment Effect (CATE) Size (Tokens)",
  #     y = "Country"
  #   ) +
  #   theme_pubr() +
  #   theme(
