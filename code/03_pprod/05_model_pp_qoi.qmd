...
plot_country_results <- function(country_qois_df) {
  ...
  make_country_ev_plot <- function(df_country) {
    ggplot(
      ...
      facet_wrap(~Partisan_Type, ncol = 2) +
      scale_fill_manual(
        values = c(
  ...
  # ============================================================
  # 3. NEW: Combined EV Plot (Option A recommended layout)
  # ============================================================
  make_combined_ev_plot <- function(ev_data) {
    ggplot(
      ev_data,
      aes(
        x = Estimate,
        y = forcats::fct_reorder(Country, Estimate, .fun = median),
        fill = Relationship_Type
      )
    ) +
      stat_halfeye(
        .width = c(0.95, 0.99),
      ...
      scale_fill_manual(
        values = c("Co" = "grey10", "Out" = "grey70", "None" = "grey40")
      ) +
      labs(
        x = expression(E(Y)),
        y = "Country",
        fill = expression(R[ij])
      ) +
  ...
  combined_ev_plot <- make_combined_ev_plot(ev_data)
  ...
  return(list(
    ...
    Combined_Country_EV_Plot = combined_ev_plot
  ))
}
```
